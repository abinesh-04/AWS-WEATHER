function getWeather() {
    ...
    fetch(currentWeatherUrl)
        ...
        .then(data => {
            ...
            trackActivity({ place: city, timestamp: new Date().toISOString() }); // Track user activity
        })
        .catch(error => {
...
function trackActivity(activity) {
    // Send user activity data to Flask backend for tracking
    ...
    .then(response => {
        if (response.ok) {
            ...
            console.error('Failed to track user activity:', response.status);
        }
    })
    .catch(error => {
